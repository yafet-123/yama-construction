...
export default function Header() {
  ...
  const navItems = [
    ...
    { label: 'Resources', href: '/resources/blog', dropdown: [
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navItems.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className="px-3 py-2 font-mono text-xs uppercase tracking-wider text-gunmetal-gray hover:text-white transition-colors"
                >
                  {item.label}
                </Link>
                {item.dropdown && (
                  <div className={`absolute top-full left-0 bg-charcoal-secondary border border-charcoal-secondary/50 min-w-[220px] py-2 transition-all duration-200 ${activeDropdown === item.label ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                    {item.dropdown.map((d) => (
                      <Link
                        key={d.label}
                        href={d.href}
                        className="block px-4 py-2 font-mono text-xs text-gunmetal-gray hover:text-white hover:bg-charcoal transition-colors"
                      >
                        {d.label}
                      </Link>
                    ))}
          ...
          <div className="container-site py-4 space-y-1">
            {navItems.map((item) => (
              <div key={item.label}>
                <Link
                  href={item.href}
                  className="block px-3 py-2 font-mono text-sm text-gunmetal-gray hover:text-white"
                  ...
                  <div className="pl-6 space-y-1">
                    {item.dropdown.map((d) => (
                      <Link
                        key={d.label}
                        href={d.href}
                        className="block px-3 py-1.5 font-mono text-xs text-gunmetal-gray hover:text-white"
                        onClick={() => setMobileMenuOpen(false)}
                      >
                        {d.label}
                      </Link>
