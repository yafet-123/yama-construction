      ...
      <section className="section-padding">
        <div className="container-site grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-4 mb-8">{project.equipment.map((eq, i) => (<li key={i}><div className="flex items-start gap-3"><span className="w-1.5 h-1.5 bg-safety-orange mt-2 flex-shrink-0" /><div><span className="font-mono text-sm text-charcoal font-semibold">{eq.name}</span><p className="font-body text-xs text-gunmetal-gray mt-1 leading-relaxed">{eq.description}</p></div></div></li>))}</ul>
